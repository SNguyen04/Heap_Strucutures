...
class RQueue {
  ...
  Node* mergeMax(Node* &heap1, Node* &heap2);
  Node* mergeMin(Node* &heap1, Node* &heap2);

  void swap(Node* &heap);
  void swap(Node* &parent, Node* &child);

  void updateNPL(Node* &aNode);
  int findMin(Node* &aNode);

  bool leftistPropertyCheckNode(Node* &aNode);

  void copy(Node* &aNode, Node* rhs);

  void print(Node* aNode) const;

  void reHeapify(Node* aNode, bool &check);

  void reStructure(Node* &host, Node* &temp);
};
#endif
